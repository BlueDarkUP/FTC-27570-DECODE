 ...
 * 仅包含底盘路径跟随逻辑
 */
@Autonomous(name = "Far0CO", group = "PedroPathing")
public class Far0CO extends OpMode {

    private Follower follower;
    private Timer opmodeTimer;
    private PathChain mainPath;

    // 定义起始姿态：根据 Path 1 的起点 (56.2, 8.08) 和起始角度 (270度，即Math.toRadians(270))
    private final Pose startPose = new Pose(56.200, 8.080, Math.toRadians(270));

    /**
     * 构建路径的方法
     * 包含从 Path 1 直接跳到 Path 5及其后续的逻辑
     */
    public void buildPaths() {
        mainPath = follower.pathBuilder()
                ...
                .addPath(
                        // Path 5 (注意：这里直接接续 Path 5，中间没有 2,3,4)
