...
@Autonomous(name = "红方开两次门", group = "Main")
public class Red2Gate extends OpMode {
    ...
    // Path15, Path16 已删除，Path14 修改为直接去射击
        ...
        switch (pathState) {
            ...
            case 302: // 【修改点】吸取完毕后，直接前往射击点
                if (!follower.isBusy()) {
                    ...
                    // 这里稍微等待一下，类似之前的逻辑，确保灯光或机械稳定
                    Thread.sleep(100);
                    setLightAnimation(C_BLUE, C_Indigo, 1.5);

                    // 执行修改后的 Path14 (Intake -> Shoot Direct)
                    follower.followPath(Path14, true); // holdEnd = true，确保射击稳定

                    // 直接跳到 Case 305 (射击)，跳过 303(开门) 和 304(回位)
                    setPathState(305);
                }
                break;

            // Case 303 (原开门动作) 已被逻辑跳过
            // Case 304 (原门到射击) 已被逻辑跳过
    ...
    public void buildPaths() {
        ...
        // 【修改点】Path 14: 现在直接从第三排吸取点 (Path13的终点) 移动到射击点
        // 起点: (126, 36), 终点: (88, 80.5)
        // 角度: 0 -> 232.3
        Path14 = follower.pathBuilder().addPath(new BezierLine(new Pose(126.000, 36.000), new Pose(88.000, 80.500)))
                .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(232.3)).build();

        // Path 15 (开门) 和 Path 16 (从门去射击) 已被删除
